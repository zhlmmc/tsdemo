            ...
            if (value !== undefined && value !== null) {
                ...
                if (rule.min !== undefined) {
                    if (typeof value === 'number' && value < rule.min) {
                        fieldErrors.push(`${key} should be greater than or equal to ${rule.min}`);
                    }
                    if (typeof value === 'string' && value.length < rule.min) {
                        fieldErrors.push(`${key} should have at least ${rule.min} characters`);
                    }
                }

                if (rule.max !== undefined) {
                    if (typeof value === 'number' && value > rule.max) {
                        fieldErrors.push(`${key} should be less than or equal to ${rule.max}`);
                    }
                    if (typeof value === 'string' && value.length > rule.max) {
                        fieldErrors.push(`${key} should have at most ${rule.max} characters`);
                    }
                }
                ...
                if (rule.nested && typeof value === 'object') {
                    ...
                    if (!nestedValidation.isValid) {
                        Object.entries(nestedValidation.errors).forEach(([nestedKey, nestedErrors]) => {
